...
export async function POST(req: Request) {
  try {
    ...
    if (lines.length > 0) {
      ...
      if (brandCandidate.length < 50) {
        extractedData.brandName = brandCandidate.replace(
          /[^a-zA-Z\s\–\-]/g,
          ""
        );
    ...
    const taglineMatch = text.match(
      /(.+)Destination(.+)|(.+)all your abroad visa(.+)/i
    );
    ...
    lines.forEach((line) => {
      serviceKeywords.forEach((kw) => {
        if (
          ...
          if (!servicesFound.includes(line)) {
            servicesFound.push(line);
          }
        }
      });
    });
    ...
    if (servicesFound.length > 3) {
      extractedData.services = servicesFound.slice(0, 6).map((title, i) => ({
        ...defaultContent.services[
          i % defaultContent.services.length
        ],
        title,
        id: `service-${i}`,
      }));
...
export async function GET() {
  try {
    ...
    const content = await Content.findOne({ isActive: true }).sort({
      createdAt: -1,
    });

    return NextResponse.json({
      success: true,
      data: content?.data || defaultContent,
    });
